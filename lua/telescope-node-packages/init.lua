...
local function remove_package(package_name)
    local package_json_path = get_package_json_path()
    if not package_json_path then
        vim.notify('No package.json file found in the current directory', vim.log.levels.WARN)
        return
    end
    local package_json = vim.fn.json_decode(vim.fn.readfile(package_json_path))
    local command
    if not package_json.node_packages or not package_json.node_packages.command then
        vim.notify("No package manager found in package.json", vim.log.levels.ERROR)
        return
    else
    ...
    Job:new({
        ...
        on_exit = function(_, return_val)
            if return_val == 0 then
                vim.notify("Package " .. package_name .. " removed successfully")
            else
                vim.notify("Failed to remove package " .. package_name, vim.log.levels.WARN)
            end
        end,
    }):start()
end
...
local function install_packages(package_names)
    ...
    local command
    if not package_json.node_packages or not package_json.node_packages.command then
        vim.notify("No package manager found in package.json", vim.log.levels.WARN)
        return
    else
        command = package_json.node_packages.command
    end
    ...
    for _, package_name in ipairs(packages) do
        ...
        Job:new({
            ...
            on_exit = function(_, return_val)
                if return_val == 0 then
                    vim.notify("Package " .. package_name .. " installed successfully")
                else
                    vim.notify("Failed to install package " .. package_name, vim.log.levels.ERROR)
                end
            end,
        }):start()
    end
